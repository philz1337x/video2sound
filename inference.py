...
def args_parse():
    ...
    config.add_argument("--seed", type=int, default=42, help="random seed")
    ...
    print(f"### Argument parsing took {round(time.time() - parsing_time,2)} seconds")
    return config
...
def build_models(config):
    ...
    print(f"Building models took {round(time.time() - build_time,2)} seconds")
...
def run_inference(config, pipe, vocoder, time_detector):
    ...
    with torch.no_grad():
        for input_video in input_list:
            ...
            audio_save_path = os.path.join(config.save_dir, "audio")
            video_save_path = os.path.join(config.save_dir, "video")
            os.makedirs(audio_save_path, exist_ok=True)
            os.makedirs(video_save_path, exist_ok=True)
            audio = audio[: int(duration * 16000)]
            audio_wav_path = os.path.join(audio_save_path, f"{name}.wav")

            sf.write(audio_wav_path, audio, 16000)
            try:
                ...
                final_video_path = os.path.join(video_save_path, f"final-movie.mp4")
                video.write_videofile(final_video_path, audio_codec='aac', codec='libx264', audio_bitrate='192k')
                print("Inference took", round(time.time() - inference_time, 2), "seconds")
